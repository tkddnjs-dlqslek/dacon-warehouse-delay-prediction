"""
Oracle-LGB-log-rawplus: same as oracle-log-v2 (best oracle) but with
v30 feat_cols + 17 raw current-state cols.
Kill if fold1 > 8.87.
"""
...
raw_avail = [c for c in RAW_EXTRA if c in fe_train_df.columns and c in fe_test_df.columns]
...
LGB_PARAMS = dict(
    objective='regression_l1', n_estimators=3000, learning_rate=0.05,
...
print("Training oracle-LGB-log-rawplus...", flush=True)
for fold_i, (tr_idx, val_idx) in enumerate(gkf.split(np.arange(len(train_raw)), groups=groups)):
    t0 = time.time()
    X_tr = make_X(X_train_base[tr_idx],
                  train_raw['lag1_logy'].values[tr_idx], train_raw['lag2_logy'].values[tr_idx],
                  train_raw['lag3_logy'].values[tr_idx], row_sc_arr[tr_idx])
    X_val = make_X(X_train_base[val_idx],
                   train_raw['lag1_logy'].values[val_idx], train_raw['lag2_logy'].values[val_idx],
                   train_raw['lag3_logy'].values[val_idx], row_sc_arr[val_idx])
    ...
    print(f"Fold {fold_i+1}: oracle-LGB-rawplus={mae:.4f}  ({time.time()-t0:.0f}s)", flush=True)
...
np.save('results/oracle_seq/oof_seqC_lgb_rawplus.npy', oof)
np.save('results/oracle_seq/test_C_lgb_rawplus.npy', test_avg)
print("Saved oof_seqC_lgb_rawplus.npy", flush=True)
...
print(f"\noracle-LGB-rawplus OOF: {oof_mae:.4f}")
print(f"oracle-LGB-lv2     OOF: {np.mean(np.abs(lv2_oof-y_true)):.4f}")
print(f"FIXED MAE: {fixed_mae:.4f}")
print(f"rawplus corr w/ lv2:    {np.corrcoef(lv2_oof, oof)[0,1]:.4f}")
print(f"rawplus corr w/ xgb_v30:{np.corrcoef(xgb_v30, oof)[0,1]:.4f}")
...
print(f"FIXED+rawplus: w={best_w:.2f}  MAE={best_m:.4f}  delta={best_m-fixed_mae:+.4f}")
...
print(f"base5+rawplus: w={best_w4:.2f}  MAE={best_m4:.4f}  delta={best_m4-np.mean(np.abs(base5-y_true)):+.4f}")
print("Done.")
