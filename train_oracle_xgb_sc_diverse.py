"""
Oracle-SC-Diverse: v30 base + suffix {mean, max, min, std} + sc_std_y.
Extends remaining oracle with volatility signal (std, min).
Kill if fold1 > 8.9.
"""
...
def compute_suffix_stats_v2(df_sorted, col, global_fill):
    """Vectorized suffix: mean, max, min, std per timestep."""
...
train_raw['sc_std_y']  = train_raw.groupby(['layout_id','scenario_id'])['y'].transform('std').fillna(0)
...
train_raw['sc_std_proxy']  = train_raw.groupby(['layout_id','scenario_id'])['mega33_y'].transform('std').fillna(0)
...
test_raw['sc_std_proxy']  = test_raw.groupby(['layout_id','scenario_id'])['mega33_y'].transform('std').fillna(0)

ORACLE_COLS = ['sc_mean_remaining','sc_max_remaining','sc_min_remaining','sc_std_remaining','sc_mean_y','sc_std_y']
PROXY_COLS  = ['sc_mean_rem_proxy', 'sc_max_rem_proxy', 'sc_min_rem_proxy', 'sc_std_rem_proxy','sc_mean_proxy','sc_std_proxy']
...
print("Training oracle-XGB-SC-Diverse...", flush=True)
for fold_i, (tr_idx, val_idx) in enumerate(gkf.split(np.arange(len(train_raw)), groups=groups)):
    ...
    print(f"Fold {fold_i+1}: oracle-XGB-SC-Diverse={mae:.4f}  ({time.time()-t0:.0f}s)", flush=True)
...
np.save('results/oracle_seq/oof_seqC_xgb_sc_diverse.npy', oof)
np.save('results/oracle_seq/test_C_xgb_sc_diverse.npy', test_avg)
...
print(f"\noracle-XGB-SC-Diverse OOF: {oof_mae:.4f}")
...
print(f"best4+SC-Diverse: w={best_w:.2f} MAE={best_m:.4f} delta={best_m-base5_mae:+.4f}")
print("Done.")
