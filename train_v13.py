"""
v13: v12 기반 + 상호작용 피처의 lag/rolling (A)
- order_per_robot, battery_risk 등 파생 피처의 시계열
- 1 seed로 빠르게 검증
"""
import sys
...
print("=== v13: v12 + 상호작용 피처 lag/rolling ===", flush=True)
...
def engineer_features(df, layout_df):
    ...
    # ━━━ v13 신규: 상호작용 피처의 lag/rolling ━━━
    interaction_cols = [
        'order_per_robot', 'battery_risk', 'congestion_x_utilization',
        'total_utilization', 'dock_pressure', 'congestion_per_robot',
        'charge_queue_per_charger', 'order_per_area',
    ]
...
pd.DataFrame({'ID': test['ID'], TARGET: final_pred}).to_csv('./submission_v13.csv', index=False)

print(f"\n{'='*60}")
print(f"v13 완료! (v12 + 상호작용 lag)")
print(f"  피처 수: {len(feature_cols)}")
print(f"  앙상블 OOF MAE: {best_mae:.4f}")
for n, w in zip(names, bw): print(f"    {n}: {w:.3f}")
print(f"  submission_v13.csv 저장 완료")
