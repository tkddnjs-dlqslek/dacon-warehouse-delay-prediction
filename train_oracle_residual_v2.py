"""
Oracle-Residual v2: lag features = residual of FIXED blend, not raw y.
- lag1_residual = (y - FIXED)[t-1], lag2_residual = (y - FIXED)[t-2]
- Residual is zero-mean, smaller scale → less distribution shift than raw y lag
- Proxy: (mega33 - FIXED)[t-1] for val/test
- Also adds sc_mean_y / sc_std_y (SC oracle) as complementary features
- Non-sequential OOF (no 25-pos loop, just direct proxy predict)
- Memory: explicit del + gc.collect to avoid peak allocation
Kill if fold1 > 8.85.
"""
...
# Map IDs to layout-sorted positions
...
# FIXED blend — load each iter_pseudo file separately
fw = dict(mega33=0.7636614598089654, rank_adj=0.1588758398901156,
          iter_r1=0.011855567572749024, iter_r2=0.034568307, iter_r3=0.031038826)

fixed_oof = fw['mega33'] * d['meta_avg_oof'][id_to_lspos]
fixed_oof += fw['rank_adj'] * np.load('results/ranking/rank_adj_oof.npy')[id_to_lspos]
r1 = np.load('results/iter_pseudo/round1_oof.npy'); fixed_oof += fw['iter_r1'] * r1[id_to_lspos]; del r1
r2 = np.load('results/iter_pseudo/round2_oof.npy'); fixed_oof += fw['iter_r2'] * r2[id_to_lspos]; del r2
r3 = np.load('results/iter_pseudo/round3_oof.npy'); fixed_oof += fw['iter_r3'] * r3[id_to_lspos]; del r3

fixed_test = fw['mega33'] * d['meta_avg_test'][test_id_to_lspos]
fixed_test += fw['rank_adj'] * np.load('results/ranking/rank_adj_test.npy')[test_id_to_lspos]
r1t = np.load('results/iter_pseudo/round1_test.npy'); fixed_test += fw['iter_r1'] * r1t[test_id_to_lspos]; del r1t
r2t = np.load('results/iter_pseudo/round2_test.npy'); fixed_test += fw['iter_r2'] * r2t[test_id_to_lspos]; del r2t
r3t = np.load('results/iter_pseudo/round3_test.npy'); fixed_test += fw['iter_r3'] * r3t[test_id_to_lspos]; del r3t
gc.collect()

# True residuals (oracle)
global_resid_mean = float(np.mean(y_true - fixed_oof))
...
# Proxy residuals for val/test: (mega33 - fixed)
...
del fixed_oof, fixed_test, mega_oof_id, mega_test_id
gc.collect()

X_base_tr = fe_train_df[feat_cols].fillna(0).values.astype(np.float32)
...
X_base_te = X_base_te[feat_cols].fillna(0).values.astype(np.float32)
del fe_train_df, fe_test_df
gc.collect()
...
def make_X(base, extra, row_sc):
    return np.hstack([base, extra.astype(np.float32), row_sc.reshape(-1,1).astype(np.float32)])
...
fw2=dict(mega33=0.7636614598089654,rank_adj=0.1588758398901156,
        iter_r1=0.011855567572749024,iter_r2=0.034568307,iter_r3=0.031038826)
fixed2 = fw2['mega33']*d2['meta_avg_oof'][id2] + fw2['rank_adj']*np.load('results/ranking/rank_adj_oof.npy')[id2]
fixed2 += fw2['iter_r1']*np.load('results/iter_pseudo/round1_oof.npy')[id2]
fixed2 += fw2['iter_r2']*np.load('results/iter_pseudo/round2_oof.npy')[id2]
fixed2 += fw2['iter_r3']*np.load('results/iter_pseudo/round3_oof.npy')[id2]
