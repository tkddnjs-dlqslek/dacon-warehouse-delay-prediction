"""
MLP2: 다른 seed + 다른 구조로 MLP 다양성 추가
  - seed=123, wider network (768-384-192)
  - MLP1과 앙상블 → 더 안정적인 MLP 예측
  - v23/v22/v24와 블렌딩 csv 생성
"""
import sys
...
t0 = time.time()
print("=== MLP2: seed=123, wider architecture ===", flush=True)
...
print(f"  피처: {len(feature_cols)}개, Phase 0 완료 ({time.time()-t0:.0f}s)", flush=True)
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
print("\n[Phase 1] MLP2 5-Fold Training (seed=123, wider)...", flush=True)

EPOCHS = 50
BATCH_SIZE = 4096
LR = 0.0008

mlp2_oof = np.zeros(len(X_all))
mlp2_test = np.zeros(len(X_test_all))
...
for fold_idx, (tr_idx, val_idx) in enumerate(folds):
    ...
    for epoch in range(EPOCHS):
        ...
        model.eval()
        with torch.no_grad():
            val_pred = np.clip(np.expm1(model(val_tensor_x).numpy()), 0, None)
    ...
    model.eval()
    with torch.no_grad():
        mlp2_oof[val_idx] = np.clip(np.expm1(model(val_tensor_x).numpy()), 0, None)
        mlp2_test += np.clip(np.expm1(model(te_tensor).numpy()), 0, None) / N_SPLITS

    print(f"  Fold {fold_idx} MAE: {best_val_mae:.4f} ({time.time()-t1:.0f}s)", flush=True)

    pickle.dump({'mlp2_oof': mlp2_oof.copy(), 'mlp2_test': mlp2_test.copy() * N_SPLITS / (fold_idx + 1)},
                open(f'{RESULT_DIR}/mlp2_fold{fold_idx}.pkl', 'wb'))
    partial_test = mlp2_test * N_SPLITS / (fold_idx + 1) if fold_idx < N_SPLITS - 1 else mlp2_test
    pd.DataFrame({'ID': test_fe['ID'], TARGET: partial_test}).to_csv(
        f'./submission_mlp2_fold{fold_idx}.csv', index=False)

mlp2_oof_mae = mean_absolute_error(y, mlp2_oof)
print(f"\n  MLP2 OOF MAE: {mlp2_oof_mae:.4f}", flush=True)

pd.DataFrame({'ID': test_fe['ID'], TARGET: mlp2_test}).to_csv('./submission_mlp2.csv', index=False)
pickle.dump({'mlp2_oof': mlp2_oof, 'mlp2_test': mlp2_test, 'mlp2_oof_mae': mlp2_oof_mae},
            open(f'{RESULT_DIR}/mlp2_final.pkl', 'wb'))
print(f"  Phase 1 완료 ({time.time()-t0:.0f}s)", flush=True)

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Phase 2: MLP1+MLP2 앙상블 + GBDT 블렌딩
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
print("\n[Phase 2] MLP1+MLP2 + GBDT 블렌딩...", flush=True)

# MLP1 로드
mlp1 = pickle.load(open(f'{RESULT_DIR}/mlp_final.pkl', 'rb'))
mlp1_oof = mlp1['mlp_oof']
mlp1_test = mlp1['mlp_test']

# MLP 앙상블 (단순 평균)
mlp_avg_oof = (mlp1_oof + mlp2_oof) / 2
mlp_avg_test = (mlp1_test + mlp2_test) / 2
mlp_avg_mae = mean_absolute_error(y, mlp_avg_oof)
print(f"  MLP1 OOF: {mlp1['mlp_oof_mae']:.4f}")
print(f"  MLP2 OOF: {mlp2_oof_mae:.4f}")
print(f"  MLP avg OOF: {mlp_avg_mae:.4f}")

from scipy.stats import pearsonr
r_12, _ = pearsonr(mlp1_test, mlp2_test)
print(f"  MLP1 vs MLP2 상관: {r_12:.4f}")
...
r_avg_v23, _ = pearsonr(mlp_avg_test, v23_test)
print(f"  MLP_avg vs v23 상관: {r_avg_v23:.4f}")
...
for ratio in np.arange(0, 0.51, 0.02):
    blended = (1 - ratio) * v23_oof + ratio * mlp_avg_oof
    mae = mean_absolute_error(y, blended)
    if mae < best_mae:
        best_mae = mae
        best_r = ratio
    if ratio in [0, 0.10, 0.20, 0.30, 0.40, 0.50]:
        print(f"    MLP {ratio:.0%}: OOF MAE={mae:.4f}", flush=True)
print(f"  -> 최적: MLP_avg {best_r:.0%} (OOF {best_mae:.4f})")

# 블렌딩 csv
for ratio in [0.05, 0.10, 0.15, 0.20, 0.25, 0.30]:
    blend = (1 - ratio) * v23_test + ratio * mlp_avg_test
    pd.DataFrame({'ID': test_fe['ID'], TARGET: blend}).to_csv(
        f'./submission_v23_mlpavg_{int(ratio*100)}.csv', index=False)

# best 3-way(v23+v22+v24) + MLP_avg
for r_mlp in [0.05, 0.10, 0.15]:
    scale = 1 - r_mlp
    blend = scale * (0.65 * v23_test + 0.20 * v22_pre_test + 0.15 * v24_test) + r_mlp * mlp_avg_test
    pd.DataFrame({'ID': test_fe['ID'], TARGET: blend}).to_csv(
        f'./submission_best3way_mlpavg_{int(r_mlp*100)}.csv', index=False)

# 5-way: v23 + v22 + v24 + MLP1 + MLP2
blend5 = 0.50 * v23_test + 0.15 * v22_pre_test + 0.10 * v24_test + 0.125 * mlp1_test + 0.125 * mlp2_test
pd.DataFrame({'ID': test_fe['ID'], TARGET: blend5}).to_csv(
    './submission_5way_blend.csv', index=False)

pickle.dump({
    'mlp2_oof': mlp2_oof, 'mlp2_test': mlp2_test,
    'mlp_avg_oof': mlp_avg_oof, 'mlp_avg_test': mlp_avg_test,
    'best_ratio': best_r, 'best_mae': best_mae,
}, open(f'{RESULT_DIR}/mlp2_blend.pkl', 'wb'))

print(f"\n{'='*60}")
print(f"MLP2 완료!")
print(f"  MLP1 OOF: {mlp1['mlp_oof_mae']:.4f}")
print(f"  MLP2 OOF: {mlp2_oof_mae:.4f}")
print(f"  MLP_avg OOF: {mlp_avg_mae:.4f}")
print(f"  MLP1 vs MLP2 상관: {r_12:.4f}")
print(f"  MLP_avg vs v23 상관: {r_avg_v23:.4f}")
print(f"  v23+MLP_avg 최적: MLP {best_r:.0%} (OOF {best_mae:.4f})")
print(f"  v23 단독 대비: {mean_absolute_error(y, v23_oof) - best_mae:+.4f}")
print(f"  submission_mlp2.csv")
print(f"  submission_v23_mlpavg_*.csv")
print(f"  submission_best3way_mlpavg_*.csv")
print(f"  submission_5way_blend.csv")
