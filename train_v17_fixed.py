...
N_SPLITS = 5
SEED = 42
CKPT_DIR = './results/v17_ckpt_fixed'
...
print(f"=== v17 fixed: 3모델 + sample weight + 후처리 ===", flush=True)
...
# Sample weight 없음 (순수 시나리오 패턴 피처 효과 확인)
sample_weight = None
print(f"   Sample weight: 없음", flush=True)
...
# 6. Submission 저장
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
pd.DataFrame({'ID': test['ID'], TARGET: global_test}).to_csv('./submission_v17.csv', index=False)
pd.DataFrame({'ID': test['ID'], TARGET: split_pred}).to_csv('./submission_v17_split.csv', index=False)
if best_strength > 0:
    cal_test = calibrate_distribution(global_test, y.values, strength=best_strength)
    pd.DataFrame({'ID': test['ID'], TARGET: cal_test}).to_csv('./submission_v17_calibrated.csv', index=False)
