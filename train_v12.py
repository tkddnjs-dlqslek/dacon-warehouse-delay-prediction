"""
v12: layout 정적 피처를 비율 피처로만 대체 (v9 기반, 1 seed)
- layout 정적 피처를 직접 넣지 않고, 운영 피처와의 비율/정규화로만 사용
"""
import sys
...
print("=== v12: layout 비율 피처로만 대체 ===", flush=True)
train = pd.read_csv('./train.csv')
test = pd.read_csv('./test.csv')
layout = pd.read_csv('./layout_info.csv')

print("1. 피처 엔지니어링 (layout = 비율 피처만)...", flush=True)
...
def engineer_features(df, layout_df):
    ...
    # layout 정적 피처 제거 (비율에만 사용하고 원본은 drop)
...
pd.DataFrame({'ID': test['ID'], TARGET: final_pred}).to_csv('./submission_v12.csv', index=False)

print(f"\n{'='*60}")
print(f"v12 완료! (layout 비율 피처로 대체)")
print(f"  피처 수: {len(feature_cols)}")
print(f"  앙상블 OOF MAE: {best_mae:.4f}")
for n, w in zip(names, bw): print(f"    {n}: {w:.3f}")
print(f"  submission_v12.csv 저장 완료")
