"""
Overnight run 2: layout_proxy 학습 → MM 배치 (oracle_NEW + layout_proxy)
"""
import subprocess, sys, os, time
os.chdir(r'C:\Users\user\Desktop\데이콘 4월')

# MM 배치를 layout_proxy용으로 수정
MM_SCRIPT = '''
...
with open('build_submissions_mm2.py', 'w', encoding='utf-8') as f:
    f.write(MM_SCRIPT)

steps = [
    ('train_oracle_layout_proxy.py', 'Layout-proxy oracle 학습'),
    ('build_submissions_mm2.py',     'MM2 batch (oracle_NEW + layout_proxy)'),
]

print("="*60)
print(f"Overnight Run 2 시작: {time.strftime('%H:%M:%S')}")
print("="*60)

for script, desc in steps:
    print(f"\n{'='*60}\n[{time.strftime('%H:%M:%S')}] {desc}")
    result = subprocess.run([sys.executable, script], capture_output=False, text=True)
    print(f"[{time.strftime('%H:%M:%S')}] {script} 완료 (exit={result.returncode})")
