"""
Meta v4c: push with mc100/mc150 + mega37/rank_adj as explicit meta features.
Base v4b features confirmed to work well.
"""
...
agree5_oof  = np.std(np.stack([bop['oracle_log_v2'], bop['oracle_xgb_combined'],
    bop['oracle_xgb_v31'], bop['oracle_xgb'], bop['oracle_xgb_rem']], axis=1), axis=1)
agree5_test = np.std(np.stack([btp['oracle_log_v2'], btp['oracle_xgb_combined'],
...
def build_meta_v4c(base_pred, bdict, pos, clf_p, agree5, agree7):
    ox = bdict['oracle_xgb']; orem = bdict['oracle_xgb_rem']
    olv2 = bdict['oracle_log_v2']
    ocb = bdict['oracle_cb']; obp = bdict['oracle_bestproxy']
    ocomb = bdict['oracle_xgb_combined']; ov31 = bdict['oracle_xgb_v31']
    mlp = bdict['mlp_deep']; mlp_s2 = bdict['mlp_s2']; mlp_g = bdict['mlp_gelu']
    m37 = bdict['mega37']; radj = bdict['rank_adj']
    ...
    return np.column_stack([
        ...
        # v4b extras
        agree7, oracle_mean,  # oracle_mean again (was oracle_mean7 in v4b)
        base_pred - oracle_mean5,
        ocb - ov31, obp - olv2,
        mlp_mean, mlp_mean - base_pred,
        mlp_s2, mlp_g,
        clf_p**0.5,
        np.log1p(np.maximum(0, base_pred - oracle_mean)),
        (base_pred > 100).astype(float),
        pos**2 / (24.0**2),
        # v4c new: mega37 and rank_adj as explicit features
        m37, radj,
        m37 - base_pred,   # mega deviation from final blend
        radj - base_pred,  # rank_adj deviation
        m37 - oracle_mean, # mega vs oracle disagreement
    ])

X_tr = build_meta_v4c(best_blend_oof, bop, train_raw['row_in_sc'].values, clf_oof, agree5_oof, agree7_oof).astype(np.float32)
X_te = build_meta_v4c(best_blend_test, btp, test_raw['row_in_sc'].values, clf_test, agree5_test, agree7_test).astype(np.float32)
print(f"Meta features v4c: {X_tr.shape[1]}")
...
prev_best = 8.34923
print(f"\n[v4c push: mc100/150 + mega features] prev_best={prev_best:.5f}", flush=True)

configs = [
    (500,  0.05, 100,  3.0, 15, 'mc100_r3'),
    (500,  0.05, 150,  3.0, 15, 'mc150_r3'),
    (500,  0.05, 200,  3.0, 15, 'mc200_r3'),   # v4b best
    (500,  0.05, 100,  5.0, 15, 'mc100_r5'),
    (500,  0.05, 200,  5.0, 15, 'mc200_r5'),   # v4b #2
    (500,  0.05, 150,  2.0, 15, 'mc150_r2'),
    (500,  0.05, 100,  2.0, 15, 'mc100_r2'),
    (800,  0.03, 150,  3.0, 15, 'lr3_mc150'),
    (500,  0.05, 200,  3.0, 31, 'mc200_nl31'),
    (500,  0.05, 100,  8.0, 15, 'mc100_r8'),   # high reg conservative
]
...
# Always save best
save_mae = best_ens_mae if best_ens_mae < best_single_mae and best_ens_test is not None else best_single_mae
if best_ens_mae < best_single_mae and best_ens_test is not None:
    save_test = best_ens_test
else:
...
fname = f'submission_meta_v4c_OOF{save_mae:.5f}.csv'
