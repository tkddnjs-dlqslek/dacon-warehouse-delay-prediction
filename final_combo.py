...
def make_pred():
    global bb_o,bb_t
    w34,dr2,dr3,wf,w_cb=0.25,-0.04,-0.02,0.72,0.12
...
dual_o=fw4_o.copy()
m1o=fw4_o>=sfw[-n1]; m2o=fw4_o>=sfw[-n2]
dual_o[m1o]=(1-w1)*fw4_o[m1o]+w1*rh_o[m1o]
dual_o[m2o]=(1-w2)*dual_o[m2o]+w2*slhm_o[m2o]
dual_o=np.clip(dual_o,0,None)
dual_t=fw4_t.copy()
m1t=fw4_t>=sft[-n1]; m2t=fw4_t>=sft[-n2]
dual_t[m1t]=(1-w1)*fw4_t[m1t]+w1*rh_t[m1t]
dual_t[m2t]=(1-w2)*dual_t[m2t]+w2*slhm_t[m2t]
...
# Combinations
inflow010=np.clip(dual_t+0.10*test_resid,0,None)
trip_inflow010=np.clip(rh_trip_t+0.10*test_resid,0,None)

m1_s_t=(fw4_t>=sft[-n1])&seen_mask; m2_s_t=(fw4_t>=sft[-n2])&seen_mask
m1_u=(fw4_t>=sft[-5000])&unseen_mask; m2_u=(fw4_t>=sft[-11000])&unseen_mask
m1f=m1_s_t|m1_u; m2f=m2_s_t|m2_u
w1a=np.where(unseen_mask,0.20,w1); w2a=np.where(unseen_mask,0.10,w2)
t_unsBst=rh_trip_t.copy()
t_unsBst[m1f]=(1-w1a[m1f])*fw4_t[m1f]+w1a[m1f]*rh_t[m1f]
t_unsBst[m2f]=(1-w2a[m2f])*t_unsBst[m2f]+w2a[m2f]*slhm_t[m2f]
t_unsBst=np.clip(t_unsBst+0.10*test_resid,0,None)

sub_tmpl=pd.read_csv('sample_submission.csv')
to_save=[
    ('FINAL_triple_inflowCalib010', trip_mae, trip_inflow010),
    ('FINAL_triple_inflowCalib010_unsBst', trip_mae, t_unsBst),
]
for name,oof,t in to_save:
    sub=sub_tmpl.copy(); sub['avg_delay_minutes_next_30m']=t
    fname=f"{name}_OOF{oof:.5f}.csv"
    sub.to_csv(fname,index=False)
    print(f"Saved {fname}: test={t.mean():.3f}  seen={t[seen_mask].mean():.3f}  unseen={t[unseen_mask].mean():.3f}")

print("\n=== COMPLETE SUBMISSION RANKING ===")
combos=[
    ('dual_gate',             dual_mae, dual_t),
    ('rh_triple',             trip_mae, rh_trip_t),
    ('dual+inflowCalib010',   dual_mae, inflow010),
    ('triple+inflowCalib010', trip_mae, trip_inflow010),
    ('triple+inflowC010+unsBst5k', trip_mae, t_unsBst),
]
print(f"{'Name':40s}  {'OOF':>9}  {'test':>8}  {'seen':>8}  {'unseen':>8}")
for name,oof,t in combos:
    print(f"  {name:38s}  {oof:>9.5f}  {t.mean():>8.3f}  {t[seen_mask].mean():>8.3f}  {t[unseen_mask].mean():>8.3f}")
