"""
Oracle-Layout: layout-level true y aggregates as oracle features.
- Oracle: mean/std/max of y across ALL rows in this layout (all scenarios, all positions)
- Proxy: same aggregates of mega33 for this layout
- Proxy error ≈ MAE / sqrt(n_per_layout) ≈ 8.40 / sqrt(1000) ≈ 0.27 (very small)
- Completely non-sequential: no lag bias, no temporal exposure issue
- Also adds layout×position interaction: mean(y[layout, pos])
Kill if fold1 > 8.85.
"""
...
train_raw['ly_max_y']   = ly.transform('max')
train_raw['ly_std_y']   = ly.transform('std').fillna(0)
train_raw['ly_min_y']   = ly.transform('min')
train_raw['lp_mean_y']  = train_raw.groupby(['layout_id','row_in_sc'])['y'].transform('mean')

# Proxy layout-level
train_raw['mega33_y'] = mega_oof_id
mg_ly = train_raw.groupby('layout_id')['mega33_y']
train_raw['ly_mean_proxy']  = mg_ly.transform('mean')
train_raw['ly_max_proxy']   = mg_ly.transform('max')
train_raw['ly_std_proxy']   = mg_ly.transform('std').fillna(0)
train_raw['ly_min_proxy']   = mg_ly.transform('min')
train_raw['lp_mean_proxy']  = train_raw.groupby(['layout_id','row_in_sc'])['mega33_y'].transform('mean')
...
test_raw['ly_max_proxy']   = te_mg_ly.transform('max')
test_raw['ly_std_proxy']   = te_mg_ly.transform('std').fillna(0)
test_raw['ly_min_proxy']   = te_mg_ly.transform('min')
test_raw['lp_mean_proxy']  = test_raw.groupby(['layout_id','row_in_sc'])['mega33_y'].transform('mean')
...
LY_ORACLE = ['ly_mean_y','ly_max_y','ly_std_y','ly_min_y','lp_mean_y']
LY_PROXY  = ['ly_mean_proxy','ly_max_proxy','ly_std_proxy','ly_min_proxy','lp_mean_proxy']
...
print("Training oracle-Layout...", flush=True)
for fold_i, (tr_idx, val_idx) in enumerate(gkf.split(np.arange(len(train_raw)), groups=groups)):
    ...
    print(f"Fold {fold_i+1}: oracle-Layout={mae:.4f}  ({time.time()-t0:.0f}s)", flush=True)
...
np.save('results/oracle_seq/oof_seqC_xgb_layout.npy', oof)
np.save('results/oracle_seq/test_C_xgb_layout.npy', test_avg)
...
print(f"\noracle-Layout OOF: {oof_mae:.4f}  FIXED: {fixed_mae:.4f}")
