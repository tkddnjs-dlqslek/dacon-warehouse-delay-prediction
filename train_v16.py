"""
v16: v14 + FI 기반 피처 축소 (1 seed, checkpoint)
- 162개 → 상위 120개로 축소
- log transform 유지
"""
import sys
...
N_SPLITS = 5
SEED = 42
CKPT_DIR = './results/v16_ckpt'
os.makedirs(CKPT_DIR, exist_ok=True)

print("=== v16: FI 피처 축소 (120개) ===", flush=True)
...
# FI 선별 (상위 120개)
print("1.5. FI 피처 선별 (상위 120개)...", flush=True)
y = train[TARGET]
...
for tr_idx, val_idx in gkf_fi.split(X_all, y, groups):
...
feature_cols = fi_df.head(120)['feature'].tolist()
print(f"   선별: {len(feature_cols)} / {len(all_feature_cols)}", flush=True)
...
pd.DataFrame({'ID': test['ID'], TARGET: final_pred}).to_csv('./submission_v16.csv', index=False)

print(f"\n{'='*60}")
print(f"v16 완료! (FI 120개 + log)")
print(f"  피처 수: {len(feature_cols)}")
print(f"  앙상블 OOF MAE: {best_mae:.4f}")
print(f"  submission_v16.csv 저장")
