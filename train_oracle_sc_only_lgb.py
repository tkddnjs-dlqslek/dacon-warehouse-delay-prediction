"""
Oracle-SC-only LGB: Same as sc_only but with LightGBM.
LGB provides complementary diversity vs XGB (different tree structure).
Kill if fold1 > 8.85.
"""
...
print("Training oracle-SC-only LGB...", flush=True)
for fold_i, (tr_idx, val_idx) in enumerate(gkf.split(np.arange(len(train_raw)), groups=groups)):
    ...
    print(f"Fold {fold_i+1}: oracle-SC-LGB={mae:.4f}  ({time.time()-t0:.0f}s)", flush=True)
...
np.save('results/oracle_seq/oof_seqC_lgb_sc_only.npy', oof)
np.save('results/oracle_seq/test_C_lgb_sc_only.npy', test_avg)
...
print(f"\noracle-SC-LGB OOF: {oof_mae:.4f}  FIXED: {fixed_mae:.4f}")
...
print(f"FIXED+SC-LGB: w={best_w:.2f} MAE={best_m:.4f} delta={best_m-fixed_mae:+.4f}")
...
print(f"base5+SC-LGB: w={best_w4:.2f} MAE={best_m4:.4f} delta={best_m4-np.mean(np.abs(base5-y_true)):+.4f}")
print("Done.")
