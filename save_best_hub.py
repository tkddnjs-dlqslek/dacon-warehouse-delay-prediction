...
# --- GP_int final model ---
print("Fitting GP_int (5 features)...")
sc_gp = StandardScaler()
gp_f = GaussianProcessRegressor(kernel=kern_int, normalize_y=True, n_restarts_optimizer=3)
gp_f.fit(sc_gp.fit_transform(X_int_tr), y)
gp_int_tp = gp_f.predict(sc_gp.transform(X_int_te))

# --- Huber final model ---
print("Fitting Huber (5 features)...")
sc_h = StandardScaler()
...
# --- GP_int(0.9) + Huber(0.1) blend ---
blend_tp = 0.9 * gp_int_tp + 0.1 * huber_tp
print(f"\nGP_int(0.9)+Huber(0.1) test predictions:")
print(f"  mean={blend_tp.mean():.3f}  std={blend_tp.std():.3f}  max={blend_tp.max():.2f}")
...
def save_corr(blend_preds, target, label, fname):
    ...
    std_v = corr_arr[unseen_mask].std()
    mx = corr_arr[unseen_mask].max(); mn = corr_arr[unseen_mask].min()
    print(f"  {label:60s}: D={du:+.4f}  std={std_v:.3f}  [{mn:.2f},{mx:.2f}]")
    sub = sub_tmpl.copy(); sub['avg_delay_minutes_next_30m'] = ct
    sub.to_csv(fname, index=False)

    # Per-layout breakdown for key test layouts
    key_lids = ['WH_283', 'WH_201', 'WH_183', 'WH_234', 'WH_241']
    print(f"  Key layout corrections:")
    for lid in key_lids:
        mask = te_lv_u['layout_id'] == lid
        if mask.any():
            idx = te_lv_u[mask].index[0]
            print(f"    {lid}: raw={blend_preds[idx-te_lv_u.index[0]]:.2f}  scaled={scaled[idx-te_lv_u.index[0]]:.2f}")

print("\n--- Saving GP_int(0.9)+Huber(0.1) LOOO=1.7032 ---")
save_corr(blend_tp, 5.5, 'GP_int(0.9)+Huber(0.1) Δ=5.5 LOOO=1.7032',
          'FINAL_NEW_oN_gpInt9hub1_5p5_OOF8.3825.csv')

# Also save with Δ=5.77 variant
print("\n--- Saving GP_int(0.9)+Huber(0.1) Δ=5.77 variant ---")
save_corr(blend_tp, 5.77, 'GP_int(0.9)+Huber(0.1) Δ=5.77 LOOO=1.7032',
          'FINAL_NEW_oN_gpInt9hub1_5p77_OOF8.3825.csv')
