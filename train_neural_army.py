"""
Neural Model Army — 6개 신규 Neural Network 학습
  1. BiLSTM (sequence)
  2. Deeper CNN with residual (sequence)
  3. MLP_wide (1024-512)
  4. MLP_deep (5-layer)
  5. MLP_resnet (skip connections)
  6. TCN (Temporal Conv Net, dilated)
  → 33-model mega-stacking
"""
import sys
...
t0 = time.time()
print("=== Neural Model Army ===", flush=True)
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Phase 0: 피처 준비 (row-level + sequence)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
print("\n[Phase 0] 피처 준비...", flush=True)
...
# Sequence version
print("  시퀀스 변환...", flush=True)
seq_cols = [c for c in train.columns if c not in ['ID', 'layout_id', 'scenario_id', TARGET]]
seq_cols = [c for c in seq_cols if c not in ['layout_type', 'aisle_width_avg', 'intersection_count',
                                               'one_way_ratio', 'pack_station_count', 'charger_count',
                                               'layout_compactness', 'zone_dispersion', 'robot_total',
...
def make_sequences(df, features, target_col=None):
...
class BiLSTM(nn.Module):
    ...
    def forward(self, x):
        out, _ = self.lstm(x)  # (batch, 25, hidden*2)
        out = self.head(out)    # (batch, 25, 1)
        return out.squeeze(-1)  # (batch, 25)

class DeepCNN(nn.Module):
    def __init__(self, n_feat):
        super().__init__()
        self.conv1 = nn.Conv1d(n_feat, 128, 3, padding=1)
    ...
    def forward(self, x):
        x = x.transpose(1, 2)  # (batch, n_feat, 25)
        x = F.relu(self.bn1(self.conv1(x)))
        r = x
        x = F.relu(self.bn2(self.conv2(x)) + r)  # residual
        x = F.relu(self.bn3(self.conv3(x)))
        r2 = x
        x = F.relu(self.bn4(self.conv4(x)) + r2)  # residual
        x = x.transpose(1, 2)  # (batch, 25, 64)
        return self.head(x).squeeze(-1)

class TCN(nn.Module):
    def __init__(self, n_feat, channels=64):
        super().__init__()
        self.c1 = nn.Conv1d(n_feat, channels, 3, padding=1, dilation=1)
        self.c2 = nn.Conv1d(channels, channels, 3, padding=2, dilation=2)
        self.c3 = nn.Conv1d(channels, channels, 3, padding=4, dilation=4)
        self.c4 = nn.Conv1d(channels, channels, 3, padding=8, dilation=8)
    ...
    def forward(self, x):
        ...
        r = x
        x = F.relu(self.bn2(self.c2(x)) + r)
        r = x
        x = F.relu(self.bn3(self.c3(x)) + r)
        r = x
        x = F.relu(self.bn4(self.c4(x)) + r)
        x = x.transpose(1, 2)
        return self.head(x).squeeze(-1)
...
class MLPResNet(nn.Module):
    def __init__(self, input_dim):
        ...
        self.block1 = nn.Sequential(nn.Linear(256, 256), nn.BatchNorm1d(256), nn.ReLU(), nn.Dropout(0.2),
                                     nn.Linear(256, 256), nn.BatchNorm1d(256))
        self.block2 = nn.Sequential(nn.Linear(256, 256), nn.BatchNorm1d(256), nn.ReLU(), nn.Dropout(0.2),
                                     nn.Linear(256, 256), nn.BatchNorm1d(256))
        self.block3 = nn.Sequential(nn.Linear(256, 256), nn.BatchNorm1d(256), nn.ReLU(), nn.Dropout(0.2),
                                     nn.Linear(256, 256), nn.BatchNorm1d(256))
...
# 학습 함수
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def train_row_model(name, model_fn, EPOCHS=40, BATCH=4096, LR=0.001):
    t1 = time.time()
    print(f"\n  [{name}] row-level training...", flush=True)
    ...
    pickle.dump({'oof': oof, 'test': tpred, 'mae': mae}, open(f'{RESULT_DIR}/{name}_final.pkl', 'wb'))
    return oof, tpred, mae

def train_seq_model(name, model_fn, EPOCHS=40, BATCH=256, LR=0.001):
    t1 = time.time()
    print(f"\n  [{name}] sequence training...", flush=True)

    # targets for sequences
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(folds_seq):
        ...
        for epoch in range(EPOCHS):
            ...
            model.eval()
            with torch.no_grad():
                vp_log = model(val_x).numpy()
                vp = np.clip(np.expm1(vp_log), 0, None)
        ...
        with torch.no_grad():
            ...
            te_x = torch.FloatTensor(X_te)
            # batch predict for test (2000 scenarios, ok in one go)
    ...
    pickle.dump({'oof': oof_rows, 'test': tpred_rows, 'mae': mae}, open(f'{RESULT_DIR}/{name}_final.pkl', 'wb'))
    return oof_rows, tpred_rows, mae

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Phase 1: 6개 모델 순차 학습
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
print("\n[Phase 1] Neural 모델 학습...", flush=True)

results = {}

# MLP variants 먼저 (빠름)
for name, model_cls in [('mlp_wide', MLPWide), ('mlp_deep', MLPDeep), ('mlp_resnet', MLPResNet)]:
    oof, tpred, mae = train_row_model(name, lambda d, cls=model_cls: cls(d))
    results[name] = {'oof': oof, 'test': tpred, 'mae': mae}

# Sequence models
for name, model_cls in [('bilstm', BiLSTM), ('deepcnn', DeepCNN), ('tcn', TCN)]:
    oof, tpred, mae = train_seq_model(name, lambda d, cls=model_cls: cls(d))
    results[name] = {'oof': oof, 'test': tpred, 'mae': mae}

pickle.dump(results, open(f'{RESULT_DIR}/neural_army.pkl', 'wb'))
print(f"\n  Phase 1 완료 ({time.time()-t0:.0f}s)", flush=True)

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Phase 2: 33-Model Mega-Stacking
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
print("\n[Phase 2] 33-model Mega-Stacking...", flush=True)
...
meta_oofs_all['lgb'] = oof
meta_tests_all['lgb'] = tpred
...
for tr_idx, val_idx in folds:
    ...
    tpred += np.expm1(m.predict(stack_test)) / N_SPLITS
oof = np.clip(oof, 0, None); tpred = np.clip(tpred, 0, None)
meta_oofs_all['xgb'] = oof
meta_tests_all['xgb'] = tpred
...
for tr_idx, val_idx in folds:
    ...
    tpred += np.expm1(m.predict(stack_test)) / N_SPLITS
oof = np.clip(oof, 0, None); tpred = np.clip(tpred, 0, None)
meta_oofs_all['cb'] = oof
meta_tests_all['cb'] = tpred
...
for name, tp in meta_tests_all.items():
    pd.DataFrame({'ID': test_fe['ID'], TARGET: tp}).to_csv(
        f'./submission_mega33_{name}.csv', index=False)
pd.DataFrame({'ID': test_fe['ID'], TARGET: meta_avg_test}).to_csv(
    './submission_mega33_avg.csv', index=False)

# 블렌딩
prev_xgb = pd.read_csv('./submission_mega27_xgb.csv')[TARGET].values
prev_avg = pd.read_csv('./submission_mega27_avg.csv')[TARGET].values
for ratio in [0.3, 0.5, 0.7]:
    blend = ratio * meta_avg_test + (1 - ratio) * prev_avg
    pd.DataFrame({'ID': test_fe['ID'], TARGET: blend}).to_csv(
        f'./submission_mega33_avg_prev_{int(ratio*100)}.csv', index=False)
    blend2 = ratio * meta_tests_all['xgb'] + (1 - ratio) * prev_xgb
    pd.DataFrame({'ID': test_fe['ID'], TARGET: blend2}).to_csv(
        f'./submission_mega33_xgb_prev_{int(ratio*100)}.csv', index=False)

pickle.dump({
    'meta_oofs': meta_oofs_all, 'meta_tests': meta_tests_all,
    'meta_avg_oof': meta_avg_oof, 'meta_avg_test': meta_avg_test,
}, open(f'{RESULT_DIR}/mega33_final.pkl', 'wb'))

print(f"\n{'='*60}")
print(f"Neural Army 완료!")
for name, data in results.items():
    print(f"  na_{name}: OOF={data['mae']:.4f}")
print(f"  mega33 LGB: {mean_absolute_error(y, meta_oofs_all['lgb']):.4f}")
print(f"  mega33 XGB: {mean_absolute_error(y, meta_oofs_all['xgb']):.4f}")
print(f"  mega33 CB:  {mean_absolute_error(y, meta_oofs_all['cb']):.4f}")
print(f"  mega33 avg: {mean_absolute_error(y, meta_avg_oof):.4f}")
print(f"  (이전 mega27 avg: 8.4279)")
