"""
v21: v14 + layout 정적 피처 유지 (layout_type만 제거)
- v14에서 layout static 제거를 주석처리 → 13개 피처 추가 유지
- OOF 0.21점 개선 확인됨
"""
import sys
...
def engineer_features(df, layout_df):
    ...
    # layout 정적 피처 유지 (layout_type만 제거)
    df = df.drop(columns=['layout_type'], errors='ignore')
...
pd.DataFrame({'ID': test['ID'], TARGET: final_pred}).to_csv('./submission_v21.csv', index=False)
...
pd.DataFrame({'ID': test['ID'], TARGET: raw_pred}).to_csv('./submission_v21_raw.csv', index=False)
pd.DataFrame({'ID': test['ID'], TARGET: log_pred}).to_csv('./submission_v21_log.csv', index=False)

print(f"\n{'='*60}")
print(f"v14 완료! (상관 제거 + log transform)")
print(f"  피처 수: {len(feature_cols)}")
print(f"  최선: {best_name} (OOF MAE: {best_mae:.4f})")
print(f"  submission_v21.csv ({best_name}) 저장")
print(f"  submission_v21_raw.csv 저장")
print(f"  submission_v21_log.csv 저장")
