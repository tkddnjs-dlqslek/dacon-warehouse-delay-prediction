"""
Oracle-XGB-newcols: v30 feat_cols + 6 truly-new raw cols (zero coverage in v30).
task_reassign_15m, avg_recovery_time, replenishment_overlap,
staff_on_floor, forklift_active_count, order_wave_count
+ their lag1, sc_mean, sc_rank (computed on-the-fly, 6*4=24 new features)
Kill if fold1 > 8.85 (must beat oracle-XGB-v30 fold1=8.8889)
"""
...
# Add 6 new cols + their lag1, sc_mean, sc_rank
new_cols_avail = [c for c in NEW_COLS if c in fe_train_df.columns]
extra_feat_cols = []
for c in new_cols_avail:
    grp_tr = fe_train_df.groupby(['layout_id','scenario_id'])[c]
    grp_te = fe_test_df.groupby(['layout_id','scenario_id'])[c]
    fe_train_df[f'{c}_lag1']    = grp_tr.shift(1).fillna(0)
    fe_test_df[f'{c}_lag1']     = grp_te.shift(1).fillna(0)
    fe_train_df[f'{c}_sc_mean'] = grp_tr.transform('mean')
    fe_test_df[f'{c}_sc_mean']  = grp_te.transform('mean')
    fe_train_df[f'{c}_sc_rank'] = grp_tr.rank(pct=True)
    fe_test_df[f'{c}_sc_rank']  = grp_te.rank(pct=True)
...
print(f"New cols: {new_cols_avail}", flush=True)
...
print("Training oracle-XGB-newcols...", flush=True)
for fold_i, (tr_idx, val_idx) in enumerate(gkf.split(np.arange(len(train_raw)), groups=groups)):
    t0 = time.time()
    X_tr  = make_X(X_train_base[tr_idx], train_raw['lag1_y'].values[tr_idx], train_raw['lag2_y'].values[tr_idx], row_sc_arr[tr_idx])
    X_val = make_X(X_train_base[val_idx], train_raw['lag1_y'].values[val_idx], train_raw['lag2_y'].values[val_idx], row_sc_arr[val_idx])
    ...
    print(f"Fold {fold_i+1}: oracle-XGB-newcols={mae:.4f}  ({time.time()-t0:.0f}s)", flush=True)
...
np.save('results/oracle_seq/oof_seqC_xgb_newcols.npy', oof)
np.save('results/oracle_seq/test_C_xgb_newcols.npy', test_avg)
print("Saved oof_seqC_xgb_newcols.npy", flush=True)
...
print(f"\noracle-XGB-newcols OOF: {oof_mae:.4f}")
print(f"oracle-XGB-v30     OOF: {np.mean(np.abs(xgb_v30-y_true)):.4f}")
print(f"FIXED MAE: {fixed_mae:.4f}")
print(f"newcols corr w/ xgb_v30: {np.corrcoef(xgb_v30,oof)[0,1]:.4f}")
print(f"newcols corr w/ lv2:     {np.corrcoef(lv2_oof,oof)[0,1]:.4f}")
...
print(f"FIXED+newcols: w={best_w:.2f}  MAE={best_m:.4f}  delta={best_m-fixed_mae:+.4f}")
...
print(f"base5+newcols: w={best_w4:.2f}  MAE={best_m4:.4f}  delta={best_m4-np.mean(np.abs(base5-y_true)):+.4f}")
print("Done.")
