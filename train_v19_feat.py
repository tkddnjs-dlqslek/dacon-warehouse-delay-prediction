"""
v19: v14 + 피처 인터랙션 5개 추가. 나머지 완전 동일.
"""
import sys
...
N_SPLITS = 5
SEED = 42
CKPT_DIR = './results/v19_feat_ckpt'
...
print(f"=== v19: v14 + 인터랙션 5개 ===", flush=True)
...
def engineer_features(df, layout_df):
    ...
    # ━━━ v19 신규: 인터랙션 5개 ━━━
    df['idle_x_order'] = df['robot_idle'] * df['order_inflow_15m']
    df['low_batt_per_robot'] = df['low_battery_ratio'] / (df['robot_total'] + 1)
    df['congestion_x_blocked'] = df['congestion_score'] * df['blocked_path_15m']
    df['order_per_busy_robot'] = df['order_inflow_15m'] / (df['robot_active'] - df['robot_idle'] + 1).clip(lower=1)
    df['pack_x_order'] = df['pack_utilization'] * df['order_inflow_15m']
...
# 학습
print(f"\n{elapsed()} 2. 학습 (raw_Huber + log_MAE + log_Huber)...", flush=True)
...
for mtype, target, transform, name in configs:
    ...
    if os.path.exists(ckpt_oof) and os.path.exists(ckpt_test):
        ...
        for fold_idx, (tr_idx, val_idx) in enumerate(folds):
            ...
            elapsed_fold = (time.time()-ft)/60
            print(f"    Fold {fold_idx+1} ({elapsed_fold:.1f}분) {elapsed()}", flush=True)
        if transform == 'log':
...
pd.DataFrame({'ID': test['ID'], TARGET: final_pred}).to_csv('./submission_v19_feat.csv', index=False)

print(f"\n{'='*60}")
print(f"v19 완료! {elapsed()}")
print(f"  피처 수: {len(feature_cols)}")
print(f"  앙상블 OOF MAE: {best_mae:.4f}")
print(f"  가중치: {dict(zip(names, bw.round(3)))}")
print(f"  pred mean={final_pred.mean():.2f}, median={np.median(final_pred):.2f}")
print(f"  submission_v19_feat.csv 저장")
