"""
Oracle-XGB with lag3 added. Late positions (17, 19, 24) show the biggest oracle gains;
more lag history at those positions may help XGB as much as it helps log_v2.
"""
import sys
...
for k in range(1, NLAGS+1):
    train_raw[f'lag{k}_y'] = grp.shift(k).fillna(global_mean)
...
oof_xgb3   = np.full(len(train_raw), np.nan)
test_xgb3_list = []

print("Training oracle-XGB-lag3...", flush=True)
for fold_i, (tr_idx, val_idx) in enumerate(gkf.split(np.arange(len(train_raw)), groups=groups)):
    ...
    foldxgb3 = np.zeros(len(val_sorted))
    for pos in range(25):
        ...
        foldxgb3[pos_mask] = np.maximum(0, model.predict(X_pos))
    oof_xgb3[val_sorted] = foldxgb3
    ...
    testxgb3 = np.zeros(len(test_raw))
    for pos in range(25):
        ...
        for k in range(1, NLAGS+1):
            if pos - k < 0:
                ...
                lags_pos.append(mega_test_sorted[test_rsc_vals == (pos-k)])
        X_pos = make_X(X_test_base[pos_idx], lags_pos, np.full(n_pos, pos))
        testxgb3[pos_idx] = np.maximum(0, model.predict(X_pos))
    test_xgb3_list.append(testxgb3)

    maeXgb3 = np.mean(np.abs(foldxgb3 - y_true[val_sorted]))
    elapsed = time.time() - t0
    print(f"Fold {fold_i+1}: oracle-XGB-lag3={maeXgb3:.4f}  ({elapsed:.0f}s)", flush=True)

test_xgb3_avg = np.mean(test_xgb3_list, axis=0)
os.makedirs('results/oracle_seq', exist_ok=True)
np.save('results/oracle_seq/oof_seqC_xgb_lag3.npy', oof_xgb3)
np.save('results/oracle_seq/test_C_xgb_lag3.npy', test_xgb3_avg)
print("Saved oof_seqC_xgb_lag3.npy and test_C_xgb_lag3.npy", flush=True)
...
xgb3_mae = np.mean(np.abs(oof_xgb3 - y2))
fixed_mae = np.mean(np.abs(fixed2 - y2))
corr_xgb3 = np.corrcoef(y2-fixed2, oof_xgb3-fixed2)[0,1]
corr_xgb1 = np.corrcoef(y2-fixed2, xgb1-fixed2)[0,1]
print(f"\noracle-XGB-lag3 MAE: {xgb3_mae:.4f}  residual_corr={corr_xgb3:.4f}", flush=True)
print(f"oracle-XGB (lag2) MAE: {np.mean(np.abs(xgb1-y2)):.4f}  residual_corr={corr_xgb1:.4f}", flush=True)
print(f"XGB-lag3 vs XGB-lag2 corr: {np.corrcoef(oof_xgb3, xgb1)[0,1]:.4f}", flush=True)
