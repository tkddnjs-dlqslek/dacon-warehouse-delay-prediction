"""
Oracle-ET-log: ExtraTrees with log(y+1) target + 3 log lags.
Key difference from RF-log: ET uses RANDOM (not optimal) split thresholds.
Random splits are less sensitive to distribution shift between training lags (true y)
and eval lags (mega33 proxy) → potentially less exposure bias than RF-log.
"""
import sys
...
oof_etlog   = np.full(len(train_raw), np.nan)
test_etlog_list = []

print("Training oracle-ET-log...", flush=True)
for fold_i, (tr_idx, val_idx) in enumerate(gkf.split(np.arange(len(train_raw)), groups=groups)):
    ...
    fold_etlog = np.zeros(len(val_sorted))
    for pos in range(25):
        ...
        fold_etlog[pos_mask] = np.maximum(0, np.expm1(pred_log))
    oof_etlog[val_sorted] = fold_etlog
    ...
    test_etlog = np.zeros(len(test_raw))
    for pos in range(25):
        ...
        pred_log = model.predict(X_pos)
        test_etlog[pos_idx] = np.maximum(0, np.expm1(pred_log))
    test_etlog_list.append(test_etlog)

    mae_fold = np.mean(np.abs(fold_etlog - y_true[val_sorted]))
    elapsed  = time.time() - t0
    print(f"Fold {fold_i+1}: oracle-ET-log={mae_fold:.4f}  ({elapsed:.0f}s)", flush=True)

test_etlog_avg = np.mean(test_etlog_list, axis=0)
os.makedirs('results/oracle_seq', exist_ok=True)
np.save('results/oracle_seq/oof_seqC_et_log.npy', oof_etlog)
np.save('results/oracle_seq/test_C_et_log.npy', test_etlog_avg)
print("Saved oof_seqC_et_log.npy, test_C_et_log.npy", flush=True)
...
etlog_mae = np.mean(np.abs(oof_etlog - y2))
fixed_mae = np.mean(np.abs(fixed2 - y2))
res_fixed = y2 - fixed2
corr_etlog = np.corrcoef(res_fixed, oof_etlog - fixed2)[0,1]
print(f"\noracle-ET-log MAE: {etlog_mae:.4f}  residual_corr={corr_etlog:.4f}", flush=True)
print(f"ET-log vs XGB corr: {np.corrcoef(oof_etlog, xgb2)[0,1]:.4f}", flush=True)
print(f"ET-log vs Lv2 corr: {np.corrcoef(oof_etlog, lv22)[0,1]:.4f}", flush=True)
best_m = fixed_mae; best_w = 0
for w in np.arange(0, 0.61, 0.04):
    bl = (1-w)*fixed2 + w*oof_etlog
