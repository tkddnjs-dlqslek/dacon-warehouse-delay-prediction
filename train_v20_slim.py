"""
v20: v14 기반 + FI top50 피처 + layout 정규화 비율 피처 4개
"""
import sys
...
N_SPLITS = 5
SEED = 42
CKPT_DIR = './results/v20_slim_ckpt'
...
print(f"=== v20: FI top50 + layout 비율 4개 ===", flush=True)
train = pd.read_csv('./train.csv')
test = pd.read_csv('./test.csv')
layout = pd.read_csv('./layout_info.csv')

print(f"{elapsed()} 1. 피처 엔지니어링 (v14 + 비율 4개)...", flush=True)
...
def engineer_features(df, layout_df):
    ...
    # ━━━ v20 신규: layout 정규화 비율 4개 ━━━
    df['congestion_per_floor'] = df['congestion_score'] / (df['floor_area_sqm'] + 1) * 1000
    df['charging_per_charger'] = df['robot_charging'] / (df['charger_count'] + 1)
    df['charge_queue_per_charger2'] = df['charge_queue_length'] / (df['charger_count'] + 1)
...
# ━━━ FI top50 선별 ━━━
print(f"\n{elapsed()} 1.5. FI top50 선별...", flush=True)
X_all = train[all_feature_cols]
fi_sum = np.zeros(len(all_feature_cols))
for tr_idx, val_idx in gkf.split(X_all, y, groups):
    m = lgb.LGBMRegressor(objective='mae', n_estimators=1000, learning_rate=0.05,
...
feature_cols = fi_df.head(50)['feature'].tolist()
print(f"   선별 피처 50개:", flush=True)
for i, row in fi_df.head(10).iterrows():
    print(f"     {row['feature']}: {row['importance']:.0f}", flush=True)
print(f"   ...", flush=True)
...
# 학습
print(f"\n{elapsed()} 2. 학습 (3모델, top50)...", flush=True)
...
pd.DataFrame({'ID': test['ID'], TARGET: final_pred}).to_csv('./submission_v20_slim.csv', index=False)

print(f"\n{'='*60}")
print(f"v20 완료! {elapsed()}")
print(f"  피처: {len(feature_cols)}개 (top50)")
print(f"  앙상블 OOF MAE: {best_mae:.4f}")
print(f"  가중치: {dict(zip(names, bw.round(3)))}")
print(f"  pred mean={final_pred.mean():.2f}, median={np.median(final_pred):.2f}")
print(f"  submission_v20_slim.csv 저장")
