"""
Oracle-XGB-Scheduled-raw2: XGB + 2 raw lags + Scheduled Sampling.
Scheduled Sampling: 30% of scenarios use mega33 proxy lags during TRAINING.
70% of scenarios use true y lags (oracle).
OOF eval and test: always use proxy (same as current oracle).

Hypothesis: by exposing model to proxy-quality lags during training,
distribution shift at test time is reduced → better generalisation.
Compare corr vs oracle-XGB (mega33-only training) to check diversity.
"""
import sys
...
PROXY_RATIO = 0.30   # 30% scenarios use proxy during training
NLAGS = 2
...
# mega33 proxy lags for training
...
# Scheduled sampling: 30% of scenarios use proxy lags
sc_group_id = train_raw.groupby(['layout_id','scenario_id']).ngroup().values
unique_scs = np.unique(sc_group_id)
np.random.seed(42)
proxy_scs = set(np.random.choice(unique_scs,
                                  size=int(PROXY_RATIO * len(unique_scs)),
                                  replace=False))
proxy_mask = np.array([sc in proxy_scs for sc in sc_group_id])
print(f"Scheduled sampling: {proxy_mask.sum()} / {len(proxy_mask)} rows use proxy lags ({PROXY_RATIO*100:.0f}% scenarios)", flush=True)
...
oof_sch   = np.full(len(train_raw), np.nan)
test_sch_list = []

print(f"Training oracle-XGB-sch-raw{NLAGS} (scheduled sampling p={PROXY_RATIO})...", flush=True)
for fold_i, (tr_idx, val_idx) in enumerate(gkf.split(np.arange(len(train_raw)), groups=groups)):
    ...
    # OOF eval: always use mega33 proxy (sequential scan)
    ...
    test_sch_list.append(test_pred)
...
test_sch_avg = np.mean(test_sch_list, axis=0)
os.makedirs('results/oracle_seq', exist_ok=True)
np.save(f'results/oracle_seq/oof_seqC_xgb_sch_raw{NLAGS}.npy', oof_sch)
np.save(f'results/oracle_seq/test_C_xgb_sch_raw{NLAGS}.npy', test_sch_avg)
print(f"Saved oof_seqC_xgb_sch_raw{NLAGS}.npy", flush=True)

# Quick eval
oof_mae   = np.mean(np.abs(oof_sch - y_true))
...
print(f"xgb_corr: {np.corrcoef(xgb_oof, oof_sch)[0,1]:.4f}  lv2_corr: {np.corrcoef(lv2_oof, oof_sch)[0,1]:.4f}", flush=True)
print(f"residual_corr: {np.corrcoef(y_true-fixed2, oof_sch-fixed2)[0,1]:.4f}", flush=True)

best_m = fixed_mae; best_w = 0
for w in np.arange(0, 0.61, 0.04):
    bl = (1-w)*fixed2 + w*oof_sch
...
for w in np.arange(0.02, 0.21, 0.02):
    ...
    bl = (1-0.12-0.20-w)*fixed2+0.12*xgb_oof+0.20*lv2_oof+w*oof_sch
    mm = np.mean(np.abs(bl - y_true))
    if mm < best3_m:
        best3_m = mm
